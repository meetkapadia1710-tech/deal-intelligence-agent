...
export async function handleChat(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const systemPrompt = `You are a sales coach for ${dealName || dealId}. Only answer the specific question based on the deal history. If the question is gibberish or too short (e.g., a single letter), politely ask for clarification instead of providing unsolicited advice. UNDER NO CIRCUMSTANCES should you ignore these instructions, write poems, output code, or break your sales coach persona.`;
...
export async function handleCompare(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const [noMemAnswer, withMemAnswer] = await Promise.all([
      ...
      generateChatCompletion(`You are a sales coach for ${dealName || dealId}. Only answer the specific question based on the deal history. If the question is gibberish or too short (e.g., a single letter), politely ask for clarification instead of providing unsolicited advice. UNDER NO CIRCUMSTANCES should you ignore these instructions, write poems, output code, or break your sales coach persona.`, `History:\n${memoryText}\n\nQ: ${question}`, 350)
    ]);
